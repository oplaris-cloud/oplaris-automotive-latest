-- 028_p47_start_mot_from_checkin_rpc.sql
-- P47.2 — SECURITY DEFINER RPC so an MOT tester can self-start an MOT
-- check-in. The check-in → job flow touches jobs (INSERT manager-only),
-- job_assignments (INSERT manager-only), and bookings (UPDATE manager-
-- only). Wrapping the flow here lets us authorise mot_tester server-side
-- with explicit role + garage checks, without relaxing any table-level
-- RLS.
--
-- Returns the newly-created job id. Runs as postgres (owner), so the
-- body must do its own authorisation.

create or replace function public.start_mot_from_checkin(p_booking_id uuid)
returns uuid
language plpgsql
security definer
...
begin
  ...
  if v_booking.service <> 'mot' then
    raise exception 'Only MOT check-ins can be self-started'
      using errcode = '22023';
  end if;
  ...
  insert into public.jobs (
    ...
    'in_diagnosis', 'mot', v_booking.source,
    'MOT — self-started' || coalesce(': ' || v_booking.notes, ''),
...
revoke all on function public.start_mot_from_checkin(uuid) from public, anon;
grant execute on function public.start_mot_from_checkin(uuid) to authenticated;
