...
| B5.2 ✓ | `6gVQJ396fffhfH7G` | Per-vehicle Job History search + chips | New shared `<FilterChips>` primitive (multi-select, URL-state-backed, `role=switch`/`aria-checked`). New `composeVehicleJobsSearchPredicate` + `searchVehicleJobs` in `src/lib/search/vehicle-jobs.ts` — vehicle scope is implicit (page route), text fan-out into `jobs.description` + `job_charges.description` + `job_parts.description` (no `invoices.description` column exists in the schema, so that part of the spec is satisfied via job_charges which IS the line-item description). Repair-type chips (mot/electrical/maintenance) filter via `bookings.service` joined on the job. Service badge appears next to each result. 7 compose + 7 chip-component unit + 6 RLS isolation tests; 450/450 unit + 168/168 RLS green.<br/>✓ a4c41d5 2026-05-02 |
| B5.3 ✓ | `6gVQJ3MHvJR3q9Rp` | In-page search on 5 list pages | Shared phone-pattern + sanitise helpers extracted to `src/lib/search/utils.ts`. New `src/lib/search/list-pages.ts` with composers for customers (+ TRADER chip wired into the existing `<FilterChips>` primitive — closes the B4 TODO marker), vehicles (+ owner-name fan-out via customer-id sub-query), stock (description / sku / location), warranties (supplier / invoice ref / claim reason / part-name fan-out via stock_item-id sub-query — note: warranties are stock-only, no customer column, so the spec's "customer name" dimension was substituted for invoice-ref + claim-reason), and messages (phone normalised + message_body + reg, multi-select message_type chips replacing the legacy single-select dropdown — uses inline state since MessagesClient is already client-stateful). 12 compose unit + 6 RLS isolation tests (5 pages + TRADER chip); 467/467 unit + 174/174 RLS green; spacing-token lint clean.<br/>✓ c79816a 2026-05-02 |
| B5.4 ✓ | `6gVQJ3Jg7X8m3FFp` | Global spotlight Cmd+K | New `src/lib/spotlight/search.ts` (server-only, `searchSpotlight()` fans out 5 entity queries in parallel under the user-session client, RLS-scoped). New `GET /api/spotlight/search?q=…` Route Handler (manager-only via `requireManager`). New `src/components/spotlight/spotlight-modal.tsx` — cmdk-based palette wrapped in `CommandDialog`, `shouldFilter={false}` so the server's ILIKE narrowing isn't re-filtered. Cmd+K (and Ctrl+K) toggles globally; Esc closes; rows route via `router.push`. Per-group caps: jobs/customers/vehicles 5, messages/stock 3 (audit cap). Mounted in `AppShell` only when caller has `manager` role — kiosk + status + /approve excluded by being outside the shell. Stock matches description/sku/location (no `supplier` column on `stock_items`); messages link to `/app/messages` (filter-driven page). New `tests/unit/setup.ts` ResizeObserver no-op stub for cmdk. 4 spotlight component + 8 search-utils unit + 5 RLS isolation tests; 479/479 unit + 179/179 RLS green.<br/>✓ 7d8c5a3 2026-05-02 |

---

## P1 sprint — Batch 6 (super_admin + SMTP, 2026-05-02)

| ID | Todoist | Title | Acceptance |
|----|---------|-------|------------|
| B6.2 ✓ | `6gVQJ3PgWxgwXV7G` | SMTP integration | Migrations 069 + 070 land per-garage SMTP credentials. `private.smtp_settings` (garage_id PK, port CHECK, password_encrypted bytea via `extensions.pgp_sym_encrypt`). Helpers: `public.upsert_smtp_settings` (super_admin gate), `public.get_smtp_settings_meta` (super_admin, password-free), `public.get_smtp_settings_for_send` (service_role-only — anon cannot reach the decrypt path even via PostgREST), `public.delete_smtp_settings`. New env var `SMTP_ENCRYPTION_KEY` (env.ts schema + .env.example). New `src/lib/email/send.ts` — nodemailer-based, fetches creds via service-role client, sends, audit-logs (`action=email_sent`, payload includes to/subject/message_id/status/error but NEVER the password or encryption key — pinned by a unit test). Added `nodemailer` + `@types/nodemailer` deps. New `/admin/settings/smtp` page (super_admin + impersonation-required) — form, write-only password field with "leave blank to keep" semantics, native checkbox for TLS, "Send test email" action. 6 RLS tests on settings access (private schema unreachable, manager refused, super_admin upsert + meta-read round-trip with no password leak, service_role decrypt round-trip, port=0 rejected at CHECK), 3 unit tests on the email helper (subject/body via stream transport, audit redacts secrets, transport failure → ok=false + status=failed). 489/489 unit + 200/200 RLS green.<br/>✓ 46a8950 2026-05-02 |
| B6.1 ✓ | `6gVwGHRHC5vRjfvp` | super_admin role (P3.5) | Migrations 065 + 066 + 067 + 068 land the full feature: `private.platform_admins` membership table; `private.is_super_admin()` reading the `app_metadata.is_super_admin` JWT claim; `custom_access_token_hook` extended to populate the claim from platform_admins membership; `private.current_garage()` honours `request.headers.x-oplaris-impersonate` ONLY when the JWT also carries is_super_admin (header alone never escalates); `public.super_admin_enter_garage(p_garage_id)` SECURITY DEFINER RPC (writes audit row); generic `private.audit_super_admin_change` AFTER trigger on customers/vehicles/jobs/bookings/invoices/job_charges/job_parts/work_logs/garages/staff/bays writes one super_admin_<op> audit row per super_admin mutation; SELECT policies extended with `OR is_super_admin()` while preserving every original predicate (jobs/bookings/job_parts/work_logs role-and-assignment scoping, customers/vehicles deleted_at filter, audit_log manager gate); write-side role gates extended for customers/vehicles/bays/jobs only (garage_id check unchanged so super_admin can only write inside the impersonated garage). Defensive `nullif(..., '')::jsonb` everywhere prevents the empty-string GUC fallout from `SET LOCAL` rollbacks. New `src/app/admin/{layout.tsx,page.tsx,actions.ts,garages/[id]/page.tsx,audit/page.tsx}` (super_admin-only chrome with red banner, garage picker, audit-log viewer); `src/lib/auth/super-admin-cookie.ts` (HMAC-signed `oplaris_impersonate` cookie, 1h TTL, HttpOnly+SameSite=Strict); `createSupabaseServerClient` injects the impersonation header on every PostgREST request when the cookie verifies; `requireSuperAdmin()` helper added; `StaffSession` gained `isSuperAdmin: boolean`. New env var `SUPER_ADMIN_COOKIE_SECRET` (added to env.ts schema + .env.example). 7 cookie + 15 RLS isolation tests added (membership unreachable, JWT-only escalation, cross-garage read, RPC gate + audit, trigger fires only for super_admin, write-policy boundary, list-garages helper); 486/486 unit + 194/194 RLS green; spacing-token lint clean; vibe-security audit clean (manager-only sidebar entry hidden — admin lives at /admin not /app — secret in env.ts schema, RLS overlays additive only, audit captured for every mutation, JWT-driven gate so a stolen cookie alone is inert).<br/>✓ 1cb6bbb 2026-05-02 |
